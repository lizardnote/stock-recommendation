...
class ExtractItems:
    ...
    def extract_items(self, filing_metadata):
        ...
        filename = os.path.join(self.raw_files_folder, filing_metadata['filename'])
        ...
        info_tables = re.findall('<infoTABLE>.*?</infoTABLE>', contents, flags=regex_flags)
        ...
        for info_table in info_tables:
            # name = re.findall('<nameOfIssuer>.*?</nameOfIssuer>', info_table, flags=regex_flags)
            # name = re.findall(r'<nameOfIssuer.*?>(.+?)</nameOfIssuer>', name[0])[0]
            name = strip_tag(info_table, 'nameOfIssuer')
            value = int(strip_tag(info_table, 'value'))
            share = int(strip_tag(info_table, 'sshPrnamt'))
            if name not in processed_tables.keys():
                processed_tables[name] = {"values": value, "shares": share}
            else:
                processed_tables[name]['values'] += value
                processed_tables[name]['shares'] += share
        ...
        # json_content['data'] = processed_tables
...
def strip_tag(html, tag):
    result = re.findall(f'<{tag}>.*?</{tag}>', html, flags=regex_flags)
    result = re.findall(fr'<{tag}.*?>(.+?)</{tag}>', result[0])[0]
    return result
...
def main():
    ...
    #     )
    processed = []
    for series in tqdm(list_of_series):
        if int(series['Date'].split('-')[0]) <= 2013:
            continue
        processed.append(extraction.process_filing(series))
